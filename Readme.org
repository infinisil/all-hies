...
This repository provides cached Nix builds for the latest stable [Haskell IDE Engine](https://github.com/haskell/haskell-ide-engine) (HIE) for all supported GHC versions. It is intended to be a successor to [hie-nix](https://github.com/domenkozar/hie-nix), which only provides a changing subset of versions. This project solves the problem of having mismatched HIE and project GHC versions, which is almost impossible to avoid if you use a globally installed HIE and have many projects.
...
**** Nix install using `~/.nixpkgs/config.nix`

Add to the `let` variables:
...
then run the following to install HIE:
